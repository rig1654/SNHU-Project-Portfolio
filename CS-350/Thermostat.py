...
#    2          Added button controls, LED state logic, and LCD updates
#------------------------------------------------------------------
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ...
        # Update LED behavior when entering heat state
        self.updateLights()
    ...
    def on_enter_cool(self):
        ...
        # Update LED behavior when entering cool state
        self.updateLights()
    ...
    def on_enter_off(self):
        ...
        # Turn off both LEDs when thermostat enters off state
        redLight.off()
        blueLight.off()
    ...
    def processTempIncButton(self):
        ...
        # Increase setpoint and update LEDs
        self.setPoint += 1
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        # Decrease setpoint and update LEDs
        self.setPoint -= 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        # LED Behavior logic depending on state and temperature
        # If current_state is heating
        if self.current_state == self.heat:
            
            # If current temperature is less than setPoint, pulse redLight
        ...
        # If current)state is cooling
        elif self.current_state == self.cool:
            
            # If current temperature is greater than setPoint, pulse blueLight
            if temp > self.setPoint:
                blueLight.pulse()
    ...
    def setupSerialOutput(self):
        ...
        # Build comma-separated string of state, current temp, and setpoint
        state_str = self.current_state.id
        temp = floor(self.getFahrenheit())
        set_point = self.setPoint
        output = f"{state_str},{temp},{set_point}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # Output current date and time to line 1 of LCD             
            ...
            if(altCounter < 6):
                ...
                temp = floor(self.getFahrenheit())  # Convert to Fahrenheit and round
		...
		# If in either Heating or Cooling state
                if state_str == "heat" or state_str == "cool":
                    lcd_line_2 = f"{state_str.upper()} Set: {set_point} F".upper()

                # If the state is 'off'
                else:
                
                    lcd_line_2 = f"{state_str.upper()} - System OFF".upper()
                    altCounter = altCounter + 1

                # Update lights every ten seconds to keep operation smooth
                if(altCounter >= 11): 
                    self.updateLights()
                    altCounter = 1
...
# Cycle thermostat state when pressed
...
# Increase setPoint temperature when pressed
...
# Decrease setPoint temperature when pressed
